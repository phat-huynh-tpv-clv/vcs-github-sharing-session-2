...
class Repo {
 constructor(){this.commits={};this.branches={};this.remote={};this.tracking={};this.head='develop';this.detached=null;this.work={};this.index={};this.seq=0;this.lanes={develop:0,main:0};this.reflog=[];this.stashes=[];this.pending=null;this.pr=null;this.make('Initial project',[],{[FILE]:'tax_rate = 0.10'});this.branches.main='A';this.index[ROUTER]='def get_booking():\n    return {"status": "ok"}';this.make('Create booking service');this.remote={main:'A',develop:'B'};this.tracking=clone(this.remote);}
 ...
 replay(queue,original,target){while(queue.length){const id=queue.shift(),c=this.commits[id],base=(c.parents.length?this.tree(c.parents[0]):{}),result=this.combine(base,this.tree(),c.tree);if(result.conflicts.length)return this.conflictState({type:'rebase',original,target,queue,current:id},result);this.make(c.message+' (rebased)',undefined,result.tree);}this.pending=null;return 'Rebase complete. The new commits have new parents and IDs; the original commits can still be found in the reflog.';}
...
const lessons=[
  {
    ...
    "key": "<strong>Git ≠ GitHub.</strong> Git manages snapshots on your machine. GitHub hosts remotes and provides Pull Requests, reviews, and CI. Committing does not mean you have pushed.",
    "steps": [
      {
      ...
      {
        "cmd": "git commit -m \"Adjust booking tax\"",
  ...
  },
  {
    ...
    "steps": [
      ...
      {
        "cmd": "git commit -m \"Booking discount\"",
  ...
  },
  {
    ...
    "steps": [
      ...
      {
        "cmd": "git commit -m \"Booking response\"",
      ...
      {
        "cmd": "git commit -m \"Update tax policy\"",
  ...
  },
  {
    ...
    "steps": [
      ...
      {
        "cmd": "git commit -m \"Merge booking API; preserve both changes\"",
  ...
  },
  {
    ...
    "steps": [
      ...
      {
        "cmd": "git commit -m \"Adjust tax\"",
        "note": "Create a new local commit."
      },
      {
        "cmd": "git commit --amend -m \"Fix booking tax calculation\"",
  ...
  },
  {
    ...
    "steps": [
      ...
      {
        "cmd": "git commit -m \"Fix urgent booking issue\"",
  ...
  },
  {
    ...
    "steps": [
      ...
      },
      {
        ...
        "note": "In this lab: fetch + merge. Local develop catches up with GitHub."
      }
    ]
  },
  {
    ...
    "steps": [
      ...
      {
        "cmd": "git commit -m \"Add booking response\"",
      ...
      {
        "cmd": "git commit -m \"Add customer context\"",
      ...
      {
        "cmd": "git commit -m \"Merge customer context into booking\"",
  ...
  },
  {
    ...
    "key": "<strong>Start here:</strong> git switch -c feature/my-idea → edit app/service.py \"tax_rate = 0.05\" → git add . → git commit -m \"Try an idea\". Enter commands in the console; click a commit to inspect its snapshot.",
    "steps": []
  }
];

function seed(kind){const r=new Repo();const run=c=>r.run(c);const commit=(f,v,m)=>{r.work[f]=v;r.index=clone(r.work);r.make(m);};if(['feature','diverged'].includes(kind)){run('git switch -c feature/booking-api');commit(FILE,'tax_rate = 0.08','Booking discount');commit(FILE,'tax_rate = 0.05','Refine booking discount');run('git switch develop');if(kind==='diverged'){commit(ROUTER,'def get_booking():\n    return {"status": "ready"}','Develop: router response');r.remote.develop=r.tip();r.tracking.develop=r.tip();}}if(kind==='conflict'){run('git switch -c feature/booking-api');commit(ROUTER,'def get_booking():\n    return {"status": "confirmed", "customer": True}','Booking: richer response');run('git switch develop');commit(ROUTER,'def get_booking_info():\n    return {"status": "ok"}','Develop: rename handler');}if(kind==='cherry'){run('git switch -c bugfix/validation');commit(FILE,'tax_rate = max(0, 0.10)','Fix negative tax validation');run('git switch develop');commit(ROUTER,'def get_booking():\n    return {"status": "ready"}','Prepare booking router');}return r;}
let repo,lessonIndex=0,cursor=0,history=[],transcript=[],selected=null,lastNote='',fitGraph=false,preparedStep=null;
...
function renderGraph(){const live=reachable(),commits=Object.values(repo.commits),depth={};for(const c of commits)depth[c.id]=c.parents.length?1+Math.max(...c.parents.map(p=>depth[p])):0;const maxLane=Math.max(...commits.map(c=>c.lane));const width=Math.max(620,commits.length*190+180),height=Math.max(255,(maxLane+1)*145+140);const pos=id=>({x:85+repo.commits[id].n*190,y:120+repo.commits[id].lane*145});let html=`<svg role="group" aria-label="Commit graph. HEAD ${esc(repo.head||'detached')} at ${esc(repo.tip())}" style="display:block;${fitGraph?'width:100%;height:auto;':''}" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><defs><marker id="arrow" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#647893"/></marker></defs>`;for(const c of commits){const q=pos(c.id);for(const p of c.parents){const a=pos(p);html+=`<path d="M ${q.x-17} ${q.y} C ${q.x-65} ${q.y}, ${a.x+65} ${a.y}, ${a.x+20} ${a.y}" fill="none" stroke="${live.has(c.id)?'#627895':'#394459'}" stroke-width="2.5" ${live.has(c.id)?'':'stroke-dasharray="5 5"'} marker-end="url(#arrow)"/>`;}}for(const c of commits){const p=pos(c.id),isHead=c.id===repo.tip(),color=isHead?'#7de2b2':live.has(c.id)?'#b5a0ff':'#68768e';const refs=[...Object.entries(repo.branches).filter(([,id])=>id===c.id).map(([name])=>name),...Object.entries(repo.tracking).filter(([,id])=>id===c.id).map(([name])=>'origin/'+name)];html+=`<g class="node" role="button" tabindex="0" data-commit="${c.id}" aria-label="Commit ${c.id}: ${esc(c.message)}"><title>${esc(c.message)}\n${esc(refs.join(', '))}</title>${isHead?`<circle cx="${p.x}" cy="${p.y}" r="24" fill="none" stroke="${color}" opacity=".3"/>`:''}<circle cx="${p.x}" cy="${p.y}" r="16" fill="${live.has(c.id)?'#1c293b':'#121b29'}" stroke="${color}" stroke-width="2.5" ${live.has(c.id)?'':'stroke-dasharray="3 3"'}/><text x="${p.x}" y="${p.y+5}" text-anchor="middle" fill="${color}" font-size="13">${c.id}</text><text x="${p.x}" y="${p.y+36}" text-anchor="middle" fill="#9baac1" font-size="10">${esc(c.message.length>22?c.message.slice(0,20)+'…':c.message)}</text>`;refs.forEach((r,i)=>{const current=r===repo.head;html+=`<text x="${p.x-25}" y="${p.y-33-i*17}" fill="${current?'#7de2b2':'#bac9de'}" font-size="11">${esc(r)}${current?' ← HEAD':''}</text>`;});html+='</g>';}html+='</svg>';$('graph').innerHTML=html;document.querySelectorAll('[data-commit]').forEach(n=>{const choose=()=>{selected=n.dataset.commit;renderInspector();};n.onclick=choose;n.onkeydown=e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();choose();}};});}
...
function renderState(){const files=(base,tree)=>{const diff=repo.changed(base,tree);return diff.length?diff.map(f=>`<div class="file">${f in tree?'M':'D'} · ${esc(f)}<code>${esc(tree[f]??'(deleted)')}</code></div>`).join(''):'<span class="empty">No changes.</span>';};$('work').innerHTML=files(repo.index,repo.work)+(repo.stashes.length?`<div class="remote-caption">▣ ${repo.stashes.length} saved stash(es)</div>`:'');$('stage').innerHTML=files(repo.tree(),repo.index);$('work-count').textContent=repo.changed(repo.index,repo.work).length;$('stage-count').textContent=repo.changed(repo.tree(),repo.index).length;$('remote').innerHTML=Object.entries(repo.remote).map(([name,id])=>`<div class="remote-line"><span>${esc(name)}</span><b>${id}</b></div>`).join('')+`<div class="remote-caption">Local origin/develop: ${esc(repo.tracking.develop)} · GitHub develop: ${esc(repo.remote.develop)}${repo.tracking.develop!==repo.remote.develop?' · Fetch needed':''}</div>`+(repo.pr?`<div class="remote-caption">PR: ${esc(repo.pr.branch)} → develop<br><b>${esc(repo.pr.status)}</b></div>`:'<div class="remote-caption">No Pull Request yet.</div>');}
function renderConflict(){const el=$('conflict'),p=repo.pending;el.hidden=!p;if(!p)return;el.innerHTML=`<h2>⚑ ${esc(p.type)} paused: combine the intent of both sides</h2><p>BASE = common ancestor / snapshot before the patch · OURS = current side · THEIRS = incoming change.</p>`+p.conflicts.map((c,i)=>`<h3>${esc(c.file)}</h3><div class="versions"><div><b>BASE</b><pre>${esc(c.base)}</pre></div><div><b>OURS</b><pre>${esc(c.ours)}</pre></div><div><b>THEIRS</b><pre>${esc(c.theirs)}</pre></div></div><label for="resolve-${i}">Desired result (edit below, then save)</label><textarea id="resolve-${i}" spellcheck="false">${esc(repo.work[c.file])}</textarea><button data-save="${i}" class="quiet">Save to working directory</button>`).join('')+`<p class="remote-caption">After saving: git add &lt;file&gt; → ${p.type==='merge'?'git commit -m "Resolve merge"':'git '+p.type+' --continue'}. Or use git ${p.type} --abort.</p>`;el.querySelectorAll('[data-save]').forEach(btn=>btn.onclick=()=>{const i=+btn.dataset.save;execute('edit '+p.conflicts[i].file+' '+JSON.stringify($('resolve-'+i).value),'Saved the conflict resolution; check and stage it before completing the operation.');});}
...
function renderCommandHint(){
 const step=preparedStep===null?null:lessons[lessonIndex].steps[preparedStep];
 $('command-hint').textContent=step?($('command').value.trim()===step.cmd?`Step ${preparedStep+1} loaded — press Run or Enter to execute. Next step only loads a command.`:'Custom command — running it will not complete the loaded demo step. Click Next step to reload its command.'):(cursor>=lessons[lessonIndex].steps.length&&lessons[lessonIndex].steps.length?'Demo complete. You can continue with your own commands.':'Click Next step to load a command, then Run or Enter to execute it.');
}
function render(){const stepsScrollTop=$('steps').scrollTop;const l=lessons[lessonIndex];$('catalog').innerHTML=lessons.map((x,i)=>`<button data-lesson="${i}" class="${i===lessonIndex?'active':''}" ${i===lessonIndex?'aria-current="page"':''}><span>${i===14?'⌘':String(i+1).padStart(2,'0')}</span>${esc(x.short)}</button>`).join('');document.querySelectorAll('[data-lesson]').forEach(b=>b.onclick=()=>load(+b.dataset.lesson));$('eyebrow').textContent=lessonIndex===14?'SANDBOX / EXPLORE YOUR IDEAS':`CHAPTER ${String(lessonIndex+1).padStart(2,'0')} / 14`;$('title').textContent=l.title;$('subtitle').textContent=l.sub;$('concept').innerHTML=l.key;$('head-chain').innerHTML=`<b>HEAD</b> → ${repo.head?esc(repo.head)+' → ':''}<b>${esc(repo.tip())}</b>${repo.head?'':' <span class="tag">DETACHED</span>'}${repo.pending?' <span class="tag">· '+repo.pending.type.toUpperCase()+' IN PROGRESS</span>':''}`;const previous=history.at(-1)?.repo;const changes=previous?Object.entries(repo.branches).filter(([name,id])=>previous.branches[name]!==id).map(([name,id])=>name+': '+(previous.branches[name]||'new')+' → '+id):[];$('changes').textContent=changes.length?changes.join(' · '):previous&&previous.head!==repo.head?'HEAD changed position; branch pointers are unchanged.':'Each commit is a snapshot. Arrows point to its parents.';$('step-count').textContent=l.steps.length?`${cursor} / ${l.steps.length} STEPS`:'ENTER COMMANDS';$('steps').innerHTML=l.steps.length?l.steps.map((st,i)=>`<div class="step ${i<cursor?'done':i===cursor?'current':''}"><span class="num">${i<cursor?'✓':String(i+1).padStart(2,'0')}</span><span class="cmd">${esc(st.cmd)}${/^(edit|lab) /.test(st.cmd)?'<small>Lab action / not a Git command</small>':''}</span></div>`).join(''):`<div class="step">Try the console below. Examples:</div>${['git switch -c feature/my-idea','edit app/service.py "tax_rate = 0.05"','git add .','git commit -m "Try an idea"','git switch develop','git merge feature/my-idea'].map(c=>`<button class="step" data-example="${esc(c)}">${esc(c)}</button>`).join('')}`;document.querySelectorAll('[data-example]').forEach(b=>b.onclick=()=>{$('command').value=b.dataset.example;$('command').focus();});$('next').disabled=cursor>=l.steps.length;$('back').disabled=!history.length;$('explanation').textContent=lastNote||'The repository is ready. Click Next step to load the first command. Predict its effect, then press Run or Enter.';$('output').textContent=transcript.join('\n\n')||'$ git status\nOn branch develop · working tree clean\nRun the demo or type help to get started.';$('help').textContent=HELP;renderCommandHint();renderQuickQuestion();renderGraph();renderInspector();renderState();renderConflict();const headNode=$('graph').querySelector('[data-commit="'+repo.tip()+'"] circle');if(headNode&&!fitGraph){const x=Number(headNode.getAttribute('cx'));$('graph').scrollLeft=Math.max(0,x-$('graph').clientWidth+180);}$('output').scrollTop=$('output').scrollHeight;const steps=$('steps');steps.scrollTop=stepsScrollTop;const current=steps.querySelector('.current');if(current){const panelRect=steps.getBoundingClientRect();const stepRect=current.getBoundingClientRect();const visibleTop=panelRect.top+steps.clientTop;const visibleBottom=visibleTop+steps.clientHeight;if(stepRect.top<visibleTop){steps.scrollTop+=stepRect.top-visibleTop;}else if(stepRect.bottom>visibleBottom){steps.scrollTop+=Math.min(stepRect.bottom-visibleBottom,stepRect.top-visibleTop);}}}
function load(index){lessonIndex=index;repo=seed(lessons[index].seed);cursor=0;history=[];transcript=[];selected=null;lastNote='';preparedStep=null;$('command').value='';$('steps').scrollTop=0;window.scrollTo(0,0);if(location.hash!=='#'+index)historyReplace(index);render();}
function historyReplace(index){window.history.replaceState(null,'','#'+index);}
function execute(command,note,advance=false){
 history.push({repo:clone(repo),cursor,transcript:[...transcript],lastNote,selected,preparedStep,commandInput:$('command').value});
 ...
 if(advance&&(success||expectedRejection)){cursor++;preparedStep=null;}
 selected=null;transcript.push('$ '+command+'\n'+out);
 if(success||expectedRejection)$('command').value='';
 render();
}
$('graph-fit').onclick=()=>{fitGraph=!fitGraph;$('graph-fit').textContent=fitGraph?'Actual size':'Fit graph';renderGraph();};$('graph-expand').onclick=()=>{document.querySelector('.visual').classList.toggle('expanded');renderGraph();};$('next').onclick=()=>{const st=lessons[lessonIndex].steps[cursor];if(!st)return;preparedStep=cursor;$('command').value=st.cmd;renderCommandHint();$('command').focus({preventScroll:true});$('command-form').scrollIntoView({block:'nearest',behavior:'auto'});};$('back').onclick=()=>{const prev=history.pop();if(!prev)return;repo=Object.assign(new Repo(),prev.repo);cursor=prev.cursor;transcript=prev.transcript;lastNote=prev.lastNote;selected=prev.selected;preparedStep=prev.preparedStep??null;$('command').value=prev.commandInput||'';render();};$('reset').onclick=()=>load(lessonIndex);$('command-form').onsubmit=e=>{e.preventDefault();const command=$('command').value.trim();if(command){const st=preparedStep===cursor?lessons[lessonIndex].steps[cursor]:null;const guided=!!st&&command===st.cmd;execute(command,guided?st.note:undefined,guided);}};$('command').addEventListener('input',renderCommandHint);$('present').onclick=()=>{document.body.classList.toggle('presentation');$('present').textContent=document.body.classList.contains('presentation')?'⛶ Exit presentation':'⛶ Present';};window.addEventListener('hashchange',()=>{const n=Number(location.hash.slice(1));if(Number.isInteger(n)&&n>=0&&n<lessons.length)load(n);});document.addEventListener('keydown',e=>{if(['INPUT','TEXTAREA','BUTTON'].includes(document.activeElement.tagName))return;if(e.key==='ArrowRight'){e.preventDefault();$('next').click();}if(e.key==='ArrowLeft'){e.preventDefault();$('back').click();}if(e.key==='Escape')document.querySelector('.visual').classList.remove('expanded');if(e.key==='Escape'&&document.body.classList.contains('presentation'))$('present').click();});load(Math.min(14,Math.max(0,Number(location.hash.slice(1))||0)));
