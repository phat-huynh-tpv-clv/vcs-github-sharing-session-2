...
function renderGraph(){
 ...
 const labels=id=>[
  ...Object.entries(repo.branches).filter(([,tip])=>tip===id).map(([name])=>({name,label:name+' [local]',current:name===repo.head,moved:!!previous&&previous.branches[name]!==id})),
  ...Object.entries(repo.tracking).filter(([,tip])=>tip===id).map(([name])=>({name:'origin/'+name,label:'origin/'+name+' [tracking]',current:false,moved:!!previous&&previous.tracking[name]!==id}))
 ];
 ...
 for(const c of commits){
  ...
  labels(c.id).forEach((ref,i)=>{html+=`<text x="${point.x}" text-anchor="middle" y="${point.y-38-i*20}" fill="${ref.moved?'#ffc58d':ref.current?'#7de2b2':'#bac9de'}" font-size="12" font-weight="${ref.moved?700:400}"><title>${esc(ref.label)}</title>${esc(ref.label)}${ref.current?' ← HEAD':''}${ref.moved?' • changed':''}</text>`;});
  html+='</g>';
 }
...
function renderChanges(){
 ...
 for(const [field,label] of [['branches','Local'],['tracking','Tracking'],['remote','Remote']]){
  for(const [name,id] of Object.entries(repo[field]))if(prev[field][name]!==id)changes.push(`${label} ${field==='tracking'?'origin/':''}${name}: ${prev[field][name]||'new'} → ${id}`);
 }
 ...
 $('changes').innerHTML=changes.length?changes.map(t=>'<div class="ref-change">'+esc(t)+'</div>').join(''):'No branch or HEAD movement. Check the file state or output below.';
}
